...
| — | `docs/data/snapshot*.json` | **Saved-forecast fallback (data, not code).** Same-origin copy of real Open-Meteo payloads for the default cities, built by `scripts/build-snapshot.js`. `02-weather-data.js` falls back to it (`showSnapshot()`) when the provider is unreachable from the visitor's network, labels it as a saved forecast, and swaps in live data when the API returns. Provider hosts are lists (`FORECAST_HOSTS`, `AIR_HOSTS`, `GEOCODE_HOSTS`) that a deployment can extend via `window.LIVE_*_HOSTS`; the last host that answered is stored under `livesky:hosts` and tried first for `LIVE_HOST_MEMORY_MS` (6 h), because a censored host hangs instead of refusing. City search has a second provider (OpenStreetMap Nominatim, `LIVE_GEOCODE_SOFT_MS` leash, serialized to respect OSM's ~1 req/s). Radius/timeouts: `LIVE_SNAPSHOT_MAX_KM` (1200 km; the notice always names the source city), `LIVE_SNAPSHOT_RACE_MS`, `LIVE_RETRY_AFTER_SNAPSHOT_MS`, `LIVE_FIRST_FETCH_TIMEOUT_MS` (first-load leash). |
