...
- `10-map-radar.js` — карты и радар RainViewer.

`docs/index.html` подключает эти **обычные (classic) script-теги** последовательно. Это важно: модули используют общую область видимости, поэтому их порядок нельзя менять. `10-map-radar.js` завершает загрузку и единственный вызывает `init()`.

`docs/js/app.js` больше не является bundle: это небольшой compatibility-loader для старых HTML-встраиваний. Ничего собирать вручную после изменения модулей не нужно.
