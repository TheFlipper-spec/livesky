...
**Надёжность данных**
- Запросы к API повторяются с задержкой, список хостов настраивается
  (`window.LIVE_FORECAST_HOSTS` / `LIVE_AIR_HOSTS` / `LIVE_GEOCODE_HOSTS`) — можно
  подставить зеркало или прокси, не трогая код
- Упавшая первая загрузка повторяется автоматически; панель ошибки снимается,
...
- **Сохранённый прогноз рядом с сайтом.** Если сеть посетителя вообще не
  ...
  прогноз: <город> · <расстояние> · <время>» и кнопкой «Повторить», а как только
  API отвечает — подменяет его живыми данными. Архив разрешён в радиусе 1200 км,
  но всегда называет город-источник; просроченный архив не показывается.
  Первый запрос ограничен 8 с, чтобы медленная сеть не оставляла экран на
  «Загрузке…». Сборка видна в консоли строкой `[LiveSky] build …`, ручная
  проверка источников — `/diag.html`.
  Приложение не зависит от одного адреса провайдера: если `api.open-meteo.com`
  закрыт вашей сетью (частая история в РФ — сайт `open-meteo.com` при этом
  открывается, это другой хост), запрос уходит на запасной адрес Open-Meteo,
  а поиск городов — на OpenStreetMap. Рабочий адрес запоминается и пробуется
  первым. Проверить, что именно доступно из вашей сети, и включить рабочий
  источник одной кнопкой можно на `/diag.html`.
